...
<div class="grid-two" style="align-items:start">
 <section class="content-card"><h2>1. Boulevard 2026-06 OAuth credentials</h2><p>Use the OAuth Credentials generated for the published Aesthetic Intel Boulevard app. Saved credentials are encrypted in Aesthetic Intel.</p>
  <div class="alert alert-info" style="margin-bottom:14px"><strong>Backend/M2M flow:</strong> Aesthetic Intel exchanges these credentials at <code>https://dashboard.boulevard.io/oauth2/token</code> and calls <code>https://dashboard.boulevard.io/api/2026-06/admin</code>. Paste the Client Secret exactly as Boulevard displays it; do not Base64-encode it yourself.</div>
  <form method="post" autocomplete="off"><?=csrf_field()?>
   <label class="field"><span>OAuth Client ID</span><input type="password" name="api_key" autocomplete="new-password" placeholder="Boulevard OAuth Client ID"><small>Leave blank to keep the saved Client ID. Current: <?=e($maskedApiKey)?></small></label>
   <label class="field"><span>OAuth Client Secret</span><input type="password" name="api_secret" autocomplete="new-password" placeholder="Boulevard OAuth Client Secret"><small>Paste the Developer Portal value exactly as shown. Leave blank to keep the saved secret. Current: <?=e($maskedApiSecret)?></small></label>
   <label class="field"><span>RUMA Boulevard Business UUID</span><input name="boulevard_business_id" value="<?=e($connection['boulevard_business_id']??'')?>" placeholder="UUID or urn:blvd:Business:..."><small>The RUMA business on which the published offline app is installed/authorized.</small></label>
